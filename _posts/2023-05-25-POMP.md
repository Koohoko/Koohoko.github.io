...
- The joint density $f_{X_{0:N} ,Y_{1:N}} (x_{0:N}, y_{1:N}; θ)$ can be written as $$ f_{X_0}(x_0;θ) \prod_{n=1}^{N} f_{X_n|X_{n−1}}(x_n|x_{n−1};θ)f_{Y_n|X_n}(y_n|x_n;θ)$$

- The marginal density for $Y_{1:N}$ evaluated at the data, $y^∗_{1:N}$, is
$$f_{Y_{1:N}}(y_{1:N}^*|\theta)= \int f_{X_{0:N} ,Y_{1:N}} (x_{0:N}, y_{1:N}^*; θ) dx_{0:N}$$
